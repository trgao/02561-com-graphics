...
function setupWebGL(canvas) {
    return WebGLUtils.setupWebGL(canvas,  { alpha: false, stencil: true });
}
...
window.onload = async function init() {
    ...
    var gvBuffer = gl.createBuffer(); // ground vertices buffer
    var gtBuffer = gl.createBuffer(); // ground tex coords buffer
    var gindexBuffer = gl.createBuffer(); // ground indices buffer
    ...
    /* Teapot program variables */
    var teapotProgram = initShaders(gl, "vertex-shader-teapot", "fragment-shader-teapot");
    gl.useProgram(teapotProgram);

    var tvBuffer = gl.createBuffer(); // teapot vertices buffer
    var tnBuffer = gl.createBuffer(); // teapot normals buffer
    var tcBuffer = gl.createBuffer(); // teapot colors buffer
    var tindexBuffer = gl.createBuffer(); // teapot indices buffer
    
    // Load teapot model drawing info
    ...
    // Load shadow frame buffer object
    ...
    function render() {
        ...
        var cameraModelView = lookAt(eye, at, up);
        ...
        var near = 0.1;
        var far = 20;
        var cameraProjection = perspective(fov, aspect, near, far);

        // Light model view
        var eye = vec3(2 * Math.sin(theta), 2, -2 + 2 * Math.cos(theta));
        var at = vec3(0.0, -0.9 + 0.9 * Math.sin(alpha), -3.0);
        var up = vec3(0.0, 1.0, 0.0);
        ...
        if (drawingInfo) {
            var modelView = translate(0.0, -0.9 + 0.9 * Math.sin(alpha), -3.0);
            var teapotModelView = mult(modelView, cameraModelView);

            /* Get ground reflector mask */
            // Set stencil operations to set mask for pixels of ground
            ...
            // Draw ground reflector without modifying color and depth buffers
            ...
            gl.uniformMatrix4fv(modelViewLoc, false, flatten(cameraModelView));
            gl.uniformMatrix4fv(projectionLoc, false, flatten(cameraProjection));
            ...
            /* Render teapot reflection */
            gl.disable(gl.BLEND);
            // Set stencil operations to draw only when in ground mask
            gl.stencilFunc(gl.EQUAL, 1, 0xFF); 
            gl.stencilOp(gl.KEEP, gl.KEEP, gl.KEEP);
            gl.useProgram(teapotProgram);

            // Draw teapot reflection
            ...
            var groundPlane = mult(transpose(inverse(mult(reflection, cameraModelView))), vec4(0.0, 1.0, 0.0, 1.0));
            var reflectionProjection = modifyProjectionMatrix(groundPlane, cameraProjection);
            gl.uniformMatrix4fv(modelViewLoc, false, flatten(reflectionModelView));
            gl.uniformMatrix4fv(projectionLoc, false, flatten(reflectionProjection));
            ...
            // Enable blending to make ground translucent
            gl.enable(gl.BLEND);
            ...
            var lightModelViewLoc = gl.getUniformLocation(teapotProgram, "lightModelView");
            var lightProjectionLoc = gl.getUniformLocation(teapotProgram, "lightProjection");
            gl.uniformMatrix4fv(modelViewLoc, false, flatten(teapotModelView));
            gl.uniformMatrix4fv(projectionLoc, false, flatten(cameraProjection));
            gl.uniformMatrix4fv(lightModelViewLoc, false, flatten(mult(lightModelView, modelView)));
            gl.uniformMatrix4fv(lightProjectionLoc, false, flatten(lightProjection));
            gl.uniform1i(gl.getUniformLocation(teapotProgram, "shadow"), 1);
