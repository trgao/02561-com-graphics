    ...
    canvas.addEventListener("click", function(ev) {
        ...
        if (drawMode == "point") {
            ...
            if (circlePoints.length == 1) {
                index -= 6;
                numPoints -= 6;
                var radius = Math.sqrt((circlePoints[0][0] - mousePos[0]) * (circlePoints[0][0] - mousePos[0]) + (circlePoints[0][1] - mousePos[1]) * (circlePoints[0][1] - mousePos[1]));
                for (var i = 0; i < 100; ++i) {
                    points.push(vec2(circlePoints[0][0] + radius * Math.cos(2 * Math.PI * i / 100), circlePoints[0][1] + radius * Math.sin(2 * Math.PI * i / 100)));
                    points.push(vec2(circlePoints[0][0] + radius * Math.cos(2 * Math.PI * (i + 1) / 100), circlePoints[0][1] + radius * Math.sin(2 * Math.PI * (i + 1) / 100)));
                    points.push(circlePoints[0]);
                    pointColors.push(currColor);
                    pointColors.push(currColor);
                    pointColors.push(circleColors[0]);
                }
                circlePoints = [];
                circleColors = [];
            } else {
